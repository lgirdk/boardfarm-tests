    ...
    <div className="px-6 py-5">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-1.5">
          <Button variant="ghost" size="sm" onClick={() => navigate("/library/suites")}>
      ...
      {/* phase strip — connected pipeline nodes */}
      <div className="mt-4 flex items-center px-2" style={{ margin: "22px 0 26px" }}>
        {PHASES.map((p, i) => {
          ...
          const curIdx = PHASES.findIndex((ph) => {
            const s = run.steps.find((st) => st.name === ph.key);
            return s?.state === "running";
          });
          const doneUpTo = curIdx >= 0 ? curIdx : PHASES.length;
          return (
            <React.Fragment key={p.key}>
              {i > 0 && (
                <div className="mx-[-2px] h-[2px] flex-1 overflow-hidden rounded-full bg-idle">
                  {i <= doneUpTo && (
                    <i className="block h-full w-full bg-gradient-to-r from-ok to-running" />
                  )}
                </div>
              )}
              <div className="flex flex-col items-center gap-[9px]" style={{ flex: "none", width: 100 }}>
                <div
                  className={cn(
                    "relative grid h-[36px] w-[36px] place-items-center rounded-full border-[1.5px] transition-all duration-300",
                    done && "border-ok/50 bg-ok/[0.09] text-ok",
                    active && "border-running bg-running/[0.12] text-running shadow-[0_0_0_6px_rgba(var(--running)/0.09),0_0_26px_rgba(var(--running)/0.2)]",
                    failed && "border-danger/50 bg-danger/[0.09] text-danger",
                    skip && "border-dashed border-idle bg-transparent text-faint",
                    !done && !active && !failed && !skip && "border-border-strong bg-surface text-faint",
                  )}
                >
                  {skip ? (
                    <span className="font-mono text-[13px] font-semibold opacity-50">—</span>
                  ) : done ? (
                    ...
                    <span className="font-mono text-[12px] font-semibold">{i + 1}</span>
                  )}
                  {active && (
                    <span className="absolute inset-[-7px] animate-[phase-ring_2.1s_ease-in-out_infinite] rounded-full border-[1.5px] border-running/[0.34]" />
                  )}
                </div>
                <div className="text-center">
                  <div
                    className={cn(
                      "text-[11.5px]",
                      active ? "font-semibold text-running" : skip || (!done && !failed) ? "text-faint" : "text-foreground",
                    )}
                  >
                    {p.label}
                  </div>
                  <div className="font-mono text-[10px] text-faint">{phaseElapsed(step)}</div>
                </div>
              </div>
            </React.Fragment>
          );
...
/** Console-aware level styles for the always-dark terminal panel */
const CONSOLE_LEVEL: Record<ConsoleLevel, string> = {
  i: "console-src",
  w: "console-warn",
  e: "console-err",
  ok: "console-ok",
...
function RunConsole({ run, tall }: { run: Run; tall?: boolean }) {
  ...
  return (
    <div>
      ...
      <div
        className={cn(
          "console-panel overflow-auto px-3 py-2.5 font-mono text-[11.5px] leading-relaxed",
      ...
      >
        {lines.map((l, i) => (
          <div key={i} className="flex gap-2.5">
            ...
            <span className="console-src shrink-0 select-none">{l.src.padEnd(10)}</span>
            <span className={cn("whitespace-pre-wrap", CONSOLE_LEVEL[l.level])}>{l.text}</span>
          </div>
        ))}
        {lines.length === 0 && <span className="console-ts">No lines.</span>}
        <div ref={endRef} />
      </div>
