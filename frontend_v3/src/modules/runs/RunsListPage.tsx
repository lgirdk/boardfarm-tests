    ...
    <div className="px-6 py-6">
      <div>
        ...
        <p className="mt-0.5 text-xs text-muted">
          Every run — test executions and pipelines, live.
        </p>
      </div>
        ...
        <table className="w-full text-left text-[12.5px]">
          <thead>
            <tr className="border-b border-border bg-surface text-[11px] tracking-[0.06em] text-faint">
              ...
              <th className="px-3.5 py-2 font-normal">what ran</th>
              ...
              >
                ...
                <td className="px-3.5 py-2.5">
                  {run.workflow === "test-run" ? (
                    <span className="font-mono text-[11.5px] text-foreground">{whatRan(run)}</span>
                  ) : (
                    <span className="text-muted">{run.workflow_title}</span>
                  )}
                </td>
                <td className="px-3.5 py-2.5">
...
/** For a test execution, the actual thing that ran; else the workflow title. */
function whatRan(run: Run): string {
  const names = (run.input?.test_names as string[] | undefined) ?? [];
  if (names.length === 1 && names[0]) return names[0];
  if (names.length > 1) return `${names.length} tests · ${String(run.input?.board_model ?? "")}`;
  return run.workflow_title;
}
