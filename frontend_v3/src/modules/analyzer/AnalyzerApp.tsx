...
/** Console-aware level styles for the always-dark evidence panel */
const CONSOLE_LEVEL: Record<EvidenceLevel, string> = {
  info: "console-src",
  ok: "console-ok",
  warn: "console-warn",
  err: "console-err",
              ...
              <div className={cn("rounded-[12px] border p-4", SEV_CARD[result.severity])}>
                ...
                {result.evidence.length > 0 && (
                  <>
                    ...
                    <div className="console-panel mt-1.5 overflow-auto rounded-[9px] px-3 py-2 font-mono text-[11px] leading-[1.7]">
                      {result.evidence.map((e, i) => (
                        <div key={i} className="flex gap-2.5">
                          <span className="console-ts shrink-0 select-none">{e.ts}</span>
                          <span className="console-src w-[72px] shrink-0 select-none">{e.src}</span>
                          <span className={cn("whitespace-pre-wrap", CONSOLE_LEVEL[e.level])}>{e.text}</span>
                        </div>
                      ))}
                    </div>
                  </>
