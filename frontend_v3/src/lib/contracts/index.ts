...
export type JiraAuthMethod = "oauth" | "pat";

/** Workspace-level Jira config — set by admin, read by users. */
export interface JiraWorkspaceConfig {
  deployment: JiraDeployment;
  /** Required for Data Center. Not set for Cloud (discovered via OAuth). */
  base_url?: string;
  /** Which auth methods users can choose from. */
  allowed_auth: JiraAuthMethod[];
  /** Whether the integration is active. */
  enabled: boolean;
}

/** Per-user Jira connection state. */
