...
export interface RunsClient {
  ...
  /** Reconstructed (mock) or streamed (real) console output for a run. */
  console(runId: string): Promise<ConsoleLine[]>;
  /** Files produced by the run's collect step (pcap, logs, serial dump). */
  runArtifacts(runId: string): Promise<RunArtifact[]>;
}
