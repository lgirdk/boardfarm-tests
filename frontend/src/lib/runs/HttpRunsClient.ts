import type { ConsoleLine, Run, RunArtifact, RunEvent, RunFilter } from "@/lib/contracts";
...
export class HttpRunsClient implements RunsClient {
  ...
  async console(runId: string): Promise<ConsoleLine[]> {
    const res = await fetch(`${this.basePath}/${encodeURIComponent(runId)}/console`);
    if (!res.ok) throw new Error(`GET console failed (HTTP ${res.status})`);
    return (await res.json()) as ConsoleLine[];
  }

  async runArtifacts(runId: string): Promise<RunArtifact[]> {
    const res = await fetch(`${this.basePath}/${encodeURIComponent(runId)}/artifacts`);
    if (!res.ok) throw new Error(`GET run artifacts failed (HTTP ${res.status})`);
