...
export function SlideOver({
  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        <Dialog.Overlay className="so-veil fixed inset-0 z-40 bg-black/50" />
        <Dialog.Content
          className={`so-panel fixed bottom-5 right-5 top-5 z-50 flex ${widthClass} flex-col overflow-hidden rounded-[16px] border border-border-strong bg-surface shadow-2xl focus:outline-none`}
